...
function mapIntegrationError(message) {
    switch (message) {
        case 'INVALID_CNPJ':
            return {
                statusCode: 400,
...
export async function registerCnpjRoutes(server) {
    ...
    server.get('/api/integrations/cnpj/:cnpj/analysis', async (request, reply) => {
        ...
        try {
            ...
            db = rlsClient(userContext.organizationId, userContext.userId);
        }
        catch {
            userContext = null;

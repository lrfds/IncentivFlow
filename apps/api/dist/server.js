...
import { LoginSchema, GetProjectsQuerySchema, CreateProjectSchema, ProjectParamsSchema, ChangePhaseSchema, ApproveValueSchema } from '@incentivflow/shared';
// Plugins
...
// RBAC hook
server.decorate('authorize', (permission) => {
...
server.post('/api/auth/login', async (request, reply) => {
    ...
    if (!result.success)
        return reply.code(400).send({ error: result.error.errors });
...
// Projects
server.get('/api/projects', { preHandler: [server.authenticate] }, async (request, reply) => {
    const q = GetProjectsQuerySchema.safeParse(request.query);
    if (!q.success)
        return reply.code(400).send({ error: q.error.errors });
...
server.post('/api/projects', { preHandler: [server.authenticate, server.authorize('project:write')] }, async (request, reply) => {
    const payload = CreateProjectSchema.safeParse(request.body);
    if (!payload.success)
        return reply.code(400).send({ error: payload.error.errors });
...
server.get('/api/projects/:id', { preHandler: [server.authenticate] }, async (request, reply) => {
    const params = ProjectParamsSchema.safeParse(request.params);
    if (!params.success)
        return reply.code(400).send({ error: params.error.errors });
...
server.post('/api/projects/:id/phase', { preHandler: [server.authenticate, server.authorize('project:write')] }, async (request, reply) => {
    const params = ProjectParamsSchema.safeParse(request.params);
    if (!params.success)
        return reply.code(400).send({ error: params.error.errors });
...
server.post('/api/projects/:id/approve', { preHandler: [server.authenticate, server.authorize('project:approve')] }, async (request, reply) => {
    const params = ProjectParamsSchema.safeParse(request.params);
    if (!params.success)
        return reply.code(400).send({ error: params.error.errors });
...
server.post('/api/projects/:id/validate-phase', { preHandler: [server.authenticate] }, async (request, reply) => {
    const params = ProjectParamsSchema.safeParse(request.params);
    if (!params.success)
        return reply.code(400).send({ error: params.error.errors });
...
// Events (audit)
server.get('/api/projects/:id/events', { preHandler: [server.authenticate, server.authorize('audit:read')] }, async (request, reply) => {
    const params = ProjectParamsSchema.safeParse(request.params);
    if (!params.success)
        return reply.code(400).send({ error: params.error.errors });
...
// Dashboard KPIs
server.get('/api/dashboard/kpis', { preHandler: [server.authenticate] }, async (request) => {
    const { organizationId } = request.user;

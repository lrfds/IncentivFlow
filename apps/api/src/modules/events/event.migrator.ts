...
export class EventMigrator {
  private static migrationMap: Map<string, EventMigration[]> = new Map();
   ...
   */
  private static validateChains(): void {
    const byType = new Map<string, EventMigration[]>();
    ...
    for (const [eventType, typeMigrations] of byType) {
      ...
      for (let i = 0; i < sorted.length - 1; i++) {
        if (sorted[i].toVersion !== sorted[i + 1].fromVersion) {
          throw new Error(
            `EVENT_MIGRATION_GAP: ${eventType} has gap between v${sorted[i].toVersion} and v${sorted[i + 1].fromVersion}`
          );
        }
      }
      ...
      if (currentVersion !== undefined) {
        ...
        if (lastMigration && lastMigration.toVersion !== currentVersion) {
          throw new Error(
            `EVENT_MIGRATION_INCOMPLETE: ${eventType} last migration goes to v${lastMigration.toVersion} but current is v${currentVersion}`
          );
        }
      }
    }
  ...
  static migrate(
    ...
    fromVersion: number,
    toVersion?: number
  ): { payload: any; migrationsApplied: string[] } {
    this.initialize();
    ...
    if (fromVersion > targetVersion) {
      throw new Error(
        `EVENT_DOWNGRADE_NOT_SUPPORTED: Cannot migrate ${eventType} from v${fromVersion} to v${toVersion}`
      );
    }
    ...
    while (currentVersion < targetVersion) {
      ...
      if (!migration) {
        throw new Error(
          `EVENT_MIGRATION_NOT_FOUND: No migration for ${eventType} from v${currentVersion}`
        );
      }
   ...
   */
  static dryRun(eventType: string, fromVersion: number, toVersion?: number): {
    steps: Array<{ from: number; to: number; description: string }>;
    targetVersion: number;
  } {
    this.initialize();

    const targetVersion = toVersion ?? CURRENT_SCHEMA_VERSIONS[eventType] ?? fromVersion;
    const steps: Array<{ from: number; to: number; description: string }> = [];
  ...
  static getStats(): {
    ...
    eventTypes: number;
    byType: Record<string, { versions: number; current: number }>;
  } {
    this.initialize();

    const byType: Record<string, { versions: number; current: number }> = {};

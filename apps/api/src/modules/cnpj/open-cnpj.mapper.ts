...
import type { OpenCnpjPartner, OpenCnpjResponse } from '@incentivflow/shared';
import { normalizeCapitalSocial, sanitizeDate, sanitizeDigits, sanitizeText } from './serpro.mapper.js';

export interface OpenCnpjFullDossier extends NormalizedCnpjDossier {
...
function mapSituacao(value: string): { codigo: string; descricao: string } {
  const normalized = sanitizeText(value);
  const upper = normalized.toUpperCase();

  if (upper.includes('ATIV')) return { codigo: '2', descricao: normalized || 'Ativa' };
  if (upper.includes('BAIX')) return { codigo: '8', descricao: normalized || 'Baixada' };
  if (upper.includes('SUSP')) return { codigo: '3', descricao: normalized || 'Suspensa' };
  if (upper.includes('INAP')) return { codigo: '1', descricao: normalized || 'Inapta' };

  return { codigo: sanitizeDigits(value) || '0', descricao: normalized };
}

function extractPhoneList(payload: Record<string, any>): Array<{ ddd?: string; numero?: string }> {
...
function normalizeSecondaryActivity(
  item: string | { codigo?: string | number | null; descricao?: string | null },
...
function mapNaturezaJuridica(payload: Record<string, any>): { codigo: string; descricao?: string } {
...
function normalizeQsaItem(item: any) {
...
export function buildOpenCnpjDossier(payload: Record<string, any>): OpenCnpjFullDossier {
  ...
  return {
    ...
    cnaesSecundarios: cnaesSecundariosRaw
      ...
      .filter((item: any): item is { codigo: string; descricao: string } => Boolean(item)),
    quadroSocios: (payload.QSA?.length ? payload.QSA : payload.qsa || [])
      .map(normalizeQsaItem)
      .filter((item: any): item is NonNullable<ReturnType<typeof normalizeQsaItem>> => Boolean(item)),
    endereco: {

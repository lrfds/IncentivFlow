...
export class ProjectService {
   ...
   */
  static async create(db: DbClient, input: CreateProjectInput) {
   ...
   */
  static async changePhase(db: DbClient, params: {
    projectId: string;
    ...
    return this.withRetry(async () => {
      ...
      return await db.$transaction(async (tx) => {
        ...
        if (targetPhase === 'POS_APROVACAO' && !project.approvedAt) {
          ...
          updates.status = 'APROVADO' as ProjectStatus;
        }
        if (targetPhase === 'CONCLUIDO' && !project.completedAt) {
          updates.completedAt = new Date();
          updates.status = 'CONCLUIDO' as ProjectStatus;
        }
   ...
   */
  static async approveValue(db: DbClient, params: {
    projectId: string;
    amount: number;
    organizationId: string;
    userId: string;
   ...
   */
  static async recordCapture(db: DbClient, params: {
    projectId: string;
    amount: number;
    organizationId: string;
    userId: string;
   ...
   */
  static async updateReadModel(db: DbClient, projectId: string) {
   ...
   */
  static async getWithHistory(db: DbClient, projectId: string) {
    const [project, documents, events, verification] = await Promise.all([
      ...
      EventStore.getStream(projectId, 0, db as any), 
      EventStore.verifyChain(projectId, db as any), 
    ]);

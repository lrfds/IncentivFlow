...
export interface PhaseTransitionRule {
  type: 'min_documents' | 'approved_value' | 'captured_percentage' | 'has_protocol' | 'has_client_approval' | 'custom';
  value?: number;
  field?: string;
  message: string;
}

export interface PhaseConfigRule {
  conditions: PhaseTransitionRule[];
...
export class PhaseEngine {
  ...
  static async validate(
    ...
    overrideVersion?: number // Para replay determinístico
  ): Promise<{ valid: boolean; errors: string[]; requiredFields?: string[]; configVersion: number }> {
    ...
    const rules = config.rules as unknown as PhaseConfigRule;
    const errors: string[] = [];
   ...
   */
  private static async validateHardcoded(
    project: any,
    toPhase: PhaseType
    ...
    switch (toPhase) {
      case 'APROVACAO_CLIENTE':
        if (docCount < 1) errors.push('Projeto deve ter ao menos 1 documento anexado');
      ...
      case 'SUBMISSAO':
        if (docCount < 2) errors.push('Mínimo 2 documentos para submissão');
        break;

      case 'ACOMPANHAMENTO':
        if (!project.protocolNumber) errors.push('Número de protocolo obrigatório');
        break;
   ...
   */
  static async seedDefaults(db: DbClient, organizationId?: string) {
    const configs = [
      {
        fromPhase: 'ELABORACAO' as PhaseType,
        toPhase: 'APROVACAO_CLIENTE' as PhaseType,
        rules: {
          conditions: [
      ...
      {
        fromPhase: 'APROVACAO_CLIENTE' as PhaseType,
        toPhase: 'SUBMISSAO' as PhaseType,
        rules: {
          conditions: [
      ...
      {
        fromPhase: 'SUBMISSAO' as PhaseType,
        toPhase: 'ACOMPANHAMENTO' as PhaseType,
        rules: {
          conditions: [
      ...
      {
        fromPhase: 'ACOMPANHAMENTO' as PhaseType,
        toPhase: 'POS_APROVACAO' as PhaseType,
        rules: {
          conditions: [
      ...
      {
        fromPhase: 'POS_APROVACAO' as PhaseType,
        toPhase: 'CONCLUIDO' as PhaseType,
        rules: {
          conditions: [

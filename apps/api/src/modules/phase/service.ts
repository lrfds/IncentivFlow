...
export interface TransitionRule {
  ...
  customValidation?: (project: any) => { valid: boolean; error?: string };
}

export const PHASE_RULES: TransitionRule[] = [
  {
    from: null,
    to: 'ELABORACAO',
...
export class PhaseService {
  static validateTransition(
    ...
    toPhase: PhaseType,
    userRole: string
  ): { valid: boolean; errors: string[]; requiredFields?: string[] } {
    const rule = PHASE_RULES.find(r => r.from === fromPhase && r.to === toPhase);
    
    if (!rule) {
      const phaseOrder: PhaseType[] = ['ELABORACAO', 'APROVACAO_CLIENTE', 'SUBMISSAO', 'ACOMPANHAMENTO', 'POS_APROVACAO', 'CONCLUIDO'];
  ...
  static async getHistory(db: DbClient, organizationId: string, projectId: string) {
    ...
    return events.map(e => ({
      id: e.id,
      fromPhase: (e.payload as any)?.fromPhase,
      toPhase: (e.payload as any)?.toPhase,
      notes: (e.payload as any)?.notes,
      transitionedAt: e.createdAt,
      userId: (e.metadata as any)?.userId,
    }));
  }

  static async getPhaseMetrics(db: DbClient, organizationId: string) {
    const events = await db.event.findMany({
    ...
    const projHistory: Record<string, any[]> = {};
    
    events.forEach(e => {
        if (!projHistory[e.aggregateId]) projHistory[e.aggregateId] = [];
        projHistory[e.aggregateId].push({ time: e.createdAt.getTime(), toPhase: (e.payload as any)?.toPhase });
    });
    ...
    Object.values(projHistory).forEach(history => {
        history.forEach((h, idx) => {
            ...
            if (next) {
                ...
                if (!phaseTimes[h.toPhase]) phaseTimes[h.toPhase] = [];

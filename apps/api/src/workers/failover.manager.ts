...
export type RegionRole = 'PRIMARY' | 'REPLICA' | 'PROMOTING' | 'DEMOTING' | 'OFFLINE';
...
export interface RegionNode {
  ...
  role: RegionRole;
  endpoint: string;
  priority: number; // Lower = higher priority for promotion
...
export interface FailoverEvent {
  id: string;
  timestamp: Date;
  type: 'HEALTH_CHECK' | 'FAILOVER_STARTED' | 'PROMOTION_COMPLETE' | 'DEMOTION_COMPLETE' | 'RECOVERY_DETECTED' | 'SPLIT_BRAIN_DETECTED' | 'CLOCK_DRIFT_WARNING';
  fromRegion: string;
  toRegion?: string;
...
export interface FailoverConfig {
  ...
  splitBrainPolicy: 'FENCE_OLD_PRIMARY' | 'REJECT_WRITES' | 'MANUAL';
}
...
export class FailoverManager {
   ...
   */
  getPrimary(): RegionNode | undefined {
    for (const region of this.regions.values()) {
      if (region.role === 'PRIMARY') return region;
    }
    return undefined;
  }
   ...
   *   3. Healthy status
   */
  selectPromotionCandidate(): RegionNode | null {
   ...
   */
  async healthCheck(regionId: string): Promise<{
    healthy: boolean;
    lagMs: number;
   ...
   */
  async executeFailover(reason: string): Promise<FailoverEvent | null> {
    ...
    console.log(
      `[FAILOVER] Complete: ${oldPrimary?.id} → ${candidate.id} (${duration}ms)`
    );
   ...
   */
  async handleRecovery(regionId: string): Promise<void> {
    const region = this.regions.get(regionId);
    if (!region) return;
   ...
   */
  private recordEvent(params: Omit<FailoverEvent, 'id' | 'timestamp'>): FailoverEvent {
   ...
   */
  getHistory(limit = 50): FailoverEvent[] {

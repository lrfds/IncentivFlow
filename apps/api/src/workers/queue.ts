...
export type JobType = 'deadline_alert' | 'daily_report' | 'audit_verification' | 'document_processing';
...
class InMemoryQueue {
  ...
  async add(type: JobType, payload: any, delayMs = 0) {
    const job: Job = {
  ...
  private async handleDeadlineAlert({ projectId }: { projectId: string }) {
    const project = await prisma.project.findUnique({
      where: { id: projectId },
      include: { client: true },
    });
    
    if (!project || !project.submissionDeadline) return;
  ...
  private async handleDailyReport({ organizationId }: { organizationId: string }) {
  ...
  private async handleAuditVerification({ organizationId }: { organizationId: string }) {
    const { AuditService } = await import('../modules/audit/service.js');
    const result = await AuditService.verifyChain(prisma as any, organizationId);
  ...
  private async handleDocumentProcessing({ documentId }: { documentId: string }) {

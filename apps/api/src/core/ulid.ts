 ...
 */
export function extractTimestamp(ulidStr: string): number {
...
export function ulidToUuid(ulidStr: string): string {
  ...
  for (let i = 0; i < ulidStr.length; i++) {
    ...
    if (charIndex === -1) throw new Error(`Invalid char: ${ulidStr[i]}`);
 ...
 */
export function regionUlid(regionId: string): { id: string; region: string; timestamp: number } {
  const id = ulid();
  return {
    id,
    region: regionId,
...
export class LamportClock {
  ...
  constructor(regionId: string, initialCounter = 0) {
   ...
   * Tick on local event
   */
  tick(): { timestamp: number; counter: number; region: string } {
    this.counter++;
    return {
      timestamp: Date.now(),
   ...
   * max(local, remote) + 1
   */
  receive(remoteCounter: number): { timestamp: number; counter: number; region: string } {
   ...
   */
  static compare(
    a: { counter: number; region: string },
    b: { counter: number; region: string }
  ): number {
